...
# ------------ Feature IV - Semantic Distance (Cos sim)--------------
...
I1_emd <- as.matrix(predict(word2vec_model, unique(first_words$word))) |>
  as.data.frame() |>
...
d_prompt1_to_word1 <- first_words |> filter(!w2v_oov) |> filter(if_all(starts_with("dim_"), ~ !is.na(.x))) |>
  get_sims(
    dim_1:dim_300, 
